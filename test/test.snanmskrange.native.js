...
var snanmskrange = tryRequire( resolve( __dirname, './../lib/snanmskrange.native.js' ) );
var opts = {
	'skip': ( snanmskrange instanceof Error )
};
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( snanmskrange.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = snanmskrange( 0, x, 1, mask, 1 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );

	v = snanmskrange( -1, x, 1, mask, 1 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
